// lib/module/reset_device/reset_device_teller_page.dart
//
// Reset Device Teller — tampilan mengikuti pola "Ganti Device Agent" di
// aplikasi CMS Medfo: cari teller (nama/User ID) -> tampil kartu detail ->
// tombol Reset Device. login_device_id/login_device_name/fcm_token diisi
// oleh proses login teller sendiri (bukan bagian kita), kita cuma baca &
...
class TellerDeviceInfo {
  ...
  factory TellerDeviceInfo.fromJson(Map<String, dynamic> json) {
    return TellerDeviceInfo(
...
// ==================== NOTIFIER ====================
class ResetDeviceTellerNotifier extends ChangeNotifier {
  ...
  Future<void> searchByKeyword(String keyword) async {
    if (keyword.isEmpty) {
      errorMessage = 'Masukkan Nama / User ID Teller';
      tellerData = null;
      notifyListeners();
      return;
    }
    ...
    try {
      final result = await TellerRepository.inquiryTellerDb(
        search: keyword,
        bprId: _bprId,
        limit: 20,
      );
      ...
      if (result['value'] == 1) {
        ...
        errorMessage = result['message'] ?? 'Teller tidak ditemukan';
        tellerData = null;
      }
    } catch (e) {
  ...
  Future<Map<String, dynamic>> resetDevice(String userid) async {
    ...
    final result = await TellerRepository.resetDevice(userid: userid);
    ...
    return {
      ...
      'message': result['message'] ?? (result['value'] == 1 ? 'Device teller berhasil direset' : 'Gagal reset device teller'),
    };
  }
...
// ==================== PAGE ====================
class ResetDeviceTellerPage extends StatefulWidget {
  const ResetDeviceTellerPage({super.key});

  @override
  State<ResetDeviceTellerPage> createState() => _ResetDeviceTellerPageState();
}

class _ResetDeviceTellerPageState extends State<ResetDeviceTellerPage> {
  late ResetDeviceTellerNotifier _notifier;
  final TextEditingController _searchController = TextEditingController();

  @override
  void initState() {
    super.initState();
    _notifier = ResetDeviceTellerNotifier();
  }
  ...
  Future<void> _resetDevice() async {
    if (_notifier.tellerData == null) return;

    final isConfirmed = await _showConfirmDialog(
      title: 'Konfirmasi Reset Device',
      message: 'Apakah Anda yakin ingin reset device teller ini?\n\n'
          'User ID: ${_notifier.tellerData!.userid}\n'
          'Nama: ${_notifier.tellerData!.nama}',
    );
    if (!isConfirmed) return;

    _showLoadingDialog();
    final result = await _notifier.resetDevice(_notifier.tellerData!.userid);
          ...
          body: Column(
            ...
            children: [
              Container(
                ...
                color: colorPrimary,
                child: const Text('Reset Device Teller',
                ...
                child: Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                            ...
                            child: Container(
                              ...
                              child: TextField(
                                ...
                                decoration: const InputDecoration(
                                  hintText: 'Masukkan Nama / User ID Teller',
                      ...
                      else if (_notifier.tellerData != null)
                        Container(
                          ...
                          child: Column(
                            children: [
                              Container(
                                ...
                                child: const Text('Detail Teller',
                                ...
                                child: Column(
                                  ...
                                  children: [
                                    ...
                                    Container(
                                      ...
                                      decoration: BoxDecoration(
                                        color: (_notifier.tellerData!.isBlokir ? Colors.red : Colors.green).withOpacity(0.1),
                                        borderRadius: BorderRadius.circular(10),
                                        border: Border.all(
                                          color: _notifier.tellerData!.isBlokir ? Colors.red : Colors.green,
                                          width: 1.5,
                                        ),
                                      ),
                                      child: Row(
                                        ...
                                        children: [
                                          Icon(
                                            _notifier.tellerData!.isBlokir ? Icons.block : Icons.check_circle,
                                            color: _notifier.tellerData!.isBlokir ? Colors.red : Colors.green,
                                            size: 18,
                                          ),
                                          const SizedBox(width: 10),
                                          Text(
                                            'Status Akun: ${_notifier.tellerData!.isBlokir ? 'Diblokir' : 'Aktif'}',
                                            style: TextStyle(
                                              fontSize: 13,
                                    ...
                                    Container(
                                      ...
                                      decoration: BoxDecoration(
                                        color: (_notifier.tellerData!.isLoggedIn ? colorPrimary : Colors.grey).withOpacity(0.12),
                                        borderRadius: BorderRadius.circular(10),
                                        border: Border.all(
                                          color: _notifier.tellerData!.isLoggedIn ? colorPrimary : Colors.grey,
                                          width: 1.5,
                                        ),
                                      ),
                                      child: Row(
                                        ...
                                        children: [
                                          Icon(
                                            _notifier.tellerData!.isLoggedIn ? Icons.smartphone : Icons.phonelink_off,
                                            color: _notifier.tellerData!.isLoggedIn ? colorPrimary : Colors.grey,
                                            size: 20,
                                          ),
                                          const SizedBox(width: 12),
                                          Text(
                                            ...
                                            style: TextStyle(
                                              fontSize: 14,
                                              fontWeight: FontWeight.bold,
                                              color: _notifier.tellerData!.isLoggedIn ? colorPrimary : Colors.grey.shade700,
                                            ),
                                          ),
                                        ],
                                      ),
                              ...
                              Padding(
                                ...
                                child: Row(
                                  ...
                                  children: [
                                    ...
                                    const SizedBox(width: 12),
                                    ElevatedButton.icon(
                                      onPressed: (_notifier.isLoading || !_notifier.tellerData!.isLoggedIn) ? null : _resetDevice,
                          ...
                          child: const Center(
                            child: Column(
                              children: [
                                ...
                                SizedBox(height: 16),
                                Text('Silakan cari teller terlebih dahulu', style: TextStyle(color: Colors.grey)),
                                SizedBox(height: 8),
