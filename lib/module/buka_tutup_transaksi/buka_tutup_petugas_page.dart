...
import 'buka_tutup_petugas_notifier.dart';
...
class _BukaTutupDataSource extends DataGridSource {
  ...
  final List<BukaTutupPetugasModel> listPetugas;
  final void Function(String id, bool value) onToggle;
  List<DataGridRow> _rows = [];

  void _buildRows() {
    // Sort berdasarkan nama terlebih dahulu
    final sortedList = List<BukaTutupPetugasModel>.from(listPetugas)
    ...
    _rows = sortedList.asMap().entries.map((entry) {
      ...
      return DataGridRow(cells: [
        ...
        DataGridCell(columnName: 'nama', value: p.nama),
        DataGridCell(columnName: 'noHp', value: p.noHp),
        DataGridCell(columnName: 'kantor', value: p.namaKantor),
        DataGridCell(columnName: 'status', value: p.transaksiKolektor),
        DataGridCell(columnName: 'toggle', value: p.id),
      ]);
    }).toList();
  }
  ...
  DataGridRowAdapter? buildRow(DataGridRow row) {
    ...
    final noHp = cells[2].value as String;
...
// ==================== HALAMAN UTAMA ====================
class BukaTutupPetugasPage extends StatefulWidget {
  const BukaTutupPetugasPage({super.key});

  @override
  State<BukaTutupPetugasPage> createState() => _BukaTutupPetugasPageState();
}

class _BukaTutupPetugasPageState extends State<BukaTutupPetugasPage> {
  _BukaTutupDataSource? _dataSource;

  @override
  Widget build(BuildContext context) {
    return ChangeNotifierProvider(
      create: (_) => BukaTutupPetugasNotifier(context: context),
      child: Consumer<BukaTutupPetugasNotifier>(
        builder: (context, notifier, child) {
          ...
          _dataSource = _BukaTutupDataSource(
            listPetugas: notifier.listPetugas,
            onToggle: (id, value) => notifier.togglePetugas(id, value),
          );
  ...
  Widget _buildToolbar(BukaTutupPetugasNotifier notifier) {
    return Container(
  ...
  Widget _buildBadge(BukaTutupPetugasNotifier notifier) {
    return Padding(
  ...
  Widget _buildBottomBar(BukaTutupPetugasNotifier notifier) {
    return Container(
