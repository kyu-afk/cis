...
class GenerateMpinNotifier extends ChangeNotifier {
  final BuildContext context;

  GenerateMpinNotifier({required this.context});
  ...
  final TextEditingController namaPetugas = TextEditingController();
  ...
  /// ID petugas yang sudah berhasil di-generate MPIN-nya.
  ...
  Future<void> generateMpin() async {
    ...
    final result = await CollectorRepository.generateMpin(
    ...
    if (result['value'] == 1) {
      ...
      _showResultDialog(
        isSuccess: true,
        message: 'MPIN berhasil digenerate untuk kolektor $_namaPetugas.\n.',
      );
    } else {
      _showResultDialog(
        isSuccess: false,
        message: result['message']?.toString().isNotEmpty == true
            ? result['message']
            : 'Generate MPIN gagal, silakan coba lagi.',
      );
    }
  }
      ...
      builder: (_) => Dialog(
        ...
        child: Container(
          ...
          child: Column(
            ...
            children: [
              Row(children: [
                const Icon(Icons.qr_code, color: colorPrimary, size: 22),
                const SizedBox(width: 10),
                const Text('Konfirmasi Generate MPIN',
                    style: TextStyle(fontSize: 17, fontWeight: FontWeight.bold)),
              ]),
              const SizedBox(height: 16),
              const Text('Apakah Anda yakin ingin generate MPIN untuk kolektor ini?',
              ...
              Container(
                ...
                child: const Row(
                  children: [
                    Icon(Icons.info_outline, size: 16, color: Color(0xFF3B6D11)),
                    SizedBox(width: 8),
                    Expanded(
                      child: Text(
                        'Generate MPIN akan menghasilkan MPIN baru untuk kolektor.',
